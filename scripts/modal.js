    ...
    return {
        createModal: function(options) {
            ...
            for (var i = 0; i < itemsArr.length; i += 1) {
				...
				var onmouseover  = item.onmouseover  || false;
				var ondrag  = item.ondrag  || false;
				...
				// Pedro - Inicio
				if (onmouseover !== false && itemType == "button") {
                    modalLabel.events.onInputOver.add(onmouseover , modalLabel);
                }
				
				if (ondrag !== false && itemType == "sprite") {
                    modalLabel.inputEnabled = true;
                    modalLabel.pixelPerfectClick = true;
                    modalLabel.priorityID = 10;
					modalLabel.input.enableDrag(true);
                }
				// Pedro - Fim
